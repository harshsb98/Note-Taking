...
def pasver(dell):
    z = conn.execute("SELECT PASSWD from Notes where ID=?", (dell,))
    for row in z:
        n = row
    if (n[0][0:] != 'None'):
        top = Toplevel()
        ...
        def ok():
            if (n[0][0:] == u.get()):
                ...
                messagebox.showinfo("Title", "password not matches")
                return(0)
    else:
        return(1)
    ...
    if(n[0][0:]!='None'):
        ...
        def ok():
            if(n[0][0:]==u.get()):
                ...
                messagebox.showinfo("Title", "password Doesn't matches")
...
def Up():
    def c():
        ...
        def save():
            ...
            re = str(vrs.get())
            input = str(e3.get("1.0", END))
            conn.execute("INSERT INTO Note(NAME,CONTENT,PRIORITY,PASSWD)VALUES(?,?,?,?)", (x, input, m, re))
            conn.commit()
            top.destroy()
